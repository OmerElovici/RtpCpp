...
static constexpr std::uint16_t swap_ushort(std::uint16_t bytes) {
...
static constexpr std::uint32_t swap_ulong(std::uint32_t bytes) {
#if defined(__clang__) || defined(__GNUC__)
...
static constexpr std::uint64_t swap_uint64(std::uint64_t bytes) {
#if defined(__clang__) || defined(__GNUC__)
...
template <typename T>
concept UintType = std::same_as<T, std::uint16_t> || std::same_as<T, std::uint32_t> ||
                   std::same_as<T, std::uint64_t>;


template <std::same_as<std::uint8_t> T>
void write_big_endian(std::uint8_t* buffer, T data) {
    *buffer = data;
}

template <std::same_as<std::uint16_t> T>
void write_big_endian(std::uint8_t* buffer, const T data) {
    buffer[0] = static_cast<std::uint8_t>((data & 0xFF'00U) >> 8);
    buffer[1] = static_cast<std::uint8_t>(data & 0x00'FFU);
}
...
template <UintType T>
T read_big_endian(const std::uint8_t* buffer) {
